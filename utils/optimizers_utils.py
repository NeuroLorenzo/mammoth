...
def meta_generate(args, model):
    meta = {}
    device = next(model.parameters()).device
    
    for n, p in model.named_parameters():
        # Inizializziamo a None per sicurezza
        p.newname = None
        
        # Identifichiamo i layer che vogliamo proteggere
        # n conterrà stringhe come 'w_in', 'w_rec', 'w_out' o 'rnn.w_rec'
        if 'w_in' in n or 'w_rec' in n or 'w_out' in n:
            # Assegniamo il nome che useremo come chiave nel dizionario meta
            p.newname = n 
            
            # Creiamo il valore xi (args.meta) per questo parametro
            # Se xi è uno scalare, basta passarlo così. 
            # Se l'ottimizzatore si aspetta un tensore (come nel paper):
            meta[p.newname] = torch.tensor(args.xi).float().to(device)
            
            print(f"Metaplasticità attivata su: {n} con xi={args.xi}")
